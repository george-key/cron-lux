...
namespace Cron.Lux.Core
{
    ...
    {
        public CronException(string msg) : base(msg)
        {

        }

        public CronException(string msg, Exception cause) : base(msg, cause)
        {
    ...
    public abstract class CronAPI
    {
        ...
        internal static Dictionary<string, string> GetAssetsInfo()
        {
            if (_systemAssets == null)
            {
                ...
                AddAsset("CRONIUM", "c56f33fc6ecfcd0c225c4ab356fee59390af8560be0e930faebe74a6daff7c9b");
                AddAsset("CRON", "602c79718b16e442de58778e148d0b1084e3b2dffd5de6b7b16cee7969282de7");
            }
        ...
        public static void EmitObject(ScriptBuilder sb, object item)
        {
            ...
            {
                throw new CronException("Unsupported contract parameter: " + item.ToString());
            }
        }
        ...
        public void GenerateInputsOutputs(UInt160 from_script_hash, IEnumerable<Transaction.Output> targets, out List<Transaction.Input> inputs, out List<Transaction.Output> outputs, decimal system_fee = 0)
        {
            ...
            if (targets == null)
            {
                ...
                if (!unspent.ContainsKey(assetName))
                    throw new CronException($"Not enough {assetName} in address {from_address}");
            ...
            foreach (var target in targets)
                if (target.scriptHash.Equals(from_script_hash))
                    throw new CronException("Target can't be same as input");
            ...
            /*
                        ...
                        {
                            var gasID = LuxUtils.ReverseHex(info["CRON"]).HexToBytes();
                            var gassources = unspent["CRON"];
        ...
        public Transaction CallContract(KeyPair key, UInt160 scriptHash, byte[] bytes, string attachSymbol = null, IEnumerable<Transaction.Output> attachTargets = null)
        {
            ...
            if (!string.IsNullOrEmpty(attachSymbol))
            {
                ...
                if (inputs.Count == 0)
                {
                    throw new CronException($"Not enough inputs for transaction");
                }
            }
        ...
        public Transaction SendAsset(KeyPair fromKey, string toAddress, string symbol, decimal amount)
        {
            ...
            {
                throw new CronException("Source and dest addresses are the same");
            }
        ...
        public Transaction WithdrawAsset(KeyPair toKey, UInt160 fromScripthash, string symbol, IEnumerable<Transaction.Output> targets, byte[] verificationScript)
        {
            ...
            GenerateInputsOutputs(toKey, "CRON", null, out gas_inputs, out gas_outputs);
        ...
        public NEP5 GetToken(string symbol)
        {
            ...
            throw new CronException("Invalid token symbol");
        }
        ...
        public Transaction DeployContract(KeyPair keys, byte[] script, byte[] parameter_list, byte return_type, ContractProperties properties, string name, string version, string author, string email, string description)
        {
            ...
            GenerateInputsOutputs(keys, "CRON", null, out inputs, out outputs, fee);
