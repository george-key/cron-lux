...
namespace Cron.Lux.Core
{
    public class NEP5
    {
        ...
        public string Name
        {
            get
            {
                ...
                {
                    throw new CronException("Api did not return a value.", e);
                }
        ...
        public decimal BalanceOf(byte[] addressHash)
        {
            ...
            {
                throw new CronException("Api did not return a value." + response);
            }
        }
    ...
    public static class TokenSale
    {
        ...
        public static Transaction MintTokens(NEP5 token, KeyPair buyer_key, string symbol, decimal amount)
        {
            ...
            attachs.Add(new Transaction.Output() { assetID = CronAPI.GetAssetID(symbol), scriptHash = token.ScriptHash, value = amount });
